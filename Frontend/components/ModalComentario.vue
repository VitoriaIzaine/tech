        ...
        <div class="modal-overlay" @click="$emit('close-modal')">
            <div class="modal" @click.stop>
                ...
                <form class="base" method="POST">
                    <input type="text" name="name" class="inputs" id="name" placeholder="name" />
                    ...
                    <button @click="postPhoto" type="submit">Cadastrar</button>
                </form>
            </div>
        </div>
    </transition>
...
export default {
    ...
    methods: {
        saveFile: async function (event) {
            this.device.file = event.files[0];
        ...
        postPhoto: async function () {
            ...
            formData.append('ativo', this.device.ativo)
            formData.append('image', this.device.file)
            this.$axios
